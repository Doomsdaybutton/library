<?php
    ...
    //global variables
    $title = $subtitle = $publisher = $description = $language = $image = $isbn13 = $isbn10 = $medium_group = $medium_index = $main_category = $volume = "";
    $categories = $firstnames = $lastnames = $authors = $isbns = $user = [];
    ...
    if(isset($_POST['submit'])){
        ...
        //lent_until
        if(isset($_SESSION['lent_until'])){
            $lent_until = $_SESSION['lent_until'];
        }
        ...
        //language
        $language = ucfirst(htmlentities($_POST['language']));
        ...
        if(isset($_SESSION['original_isbn13'])){
            $original_isbn13 = $_SESSION['original_isbn13'];
        } else {
            $original_isbn13 = "";
        }
        if(isset($_SESSION['original_isbn10'])){
            $original_isbn10 = $_SESSION['original_isbn10'];
        } else {
            $original_isbn10 = "";
        }
        ...
        if(strpos($_POST['submit'], "emove_author") == 1){
            $remove_author_position = (int)substr($_POST['submit'], 13);
            unset($authors[$remove_author_position]);
        ...
        if($isbn13 != "" && $_POST['submit'] == "Edit Medium"){
            //already exists
            if($isbn13 != $original_isbn13){
        ...
        if($isbn10 != "" && !$isbn13_error && $_POST['submit'] == "Edit Medium"){
            //already exists
            if($isbn10 != $original_isbn10){
        ...
        if(!$isbn13_error && !$isbn10_error && $_POST['submit'] == "Edit Medium"){
            ...
            if(empty($title)){
                ...
                if(!$empty_author_error && !$volume_error && !$medium_group_error){
                    //string building
                    ...
                    if(!$image_file_exists && $image != ""){
                        $query = "UPDATE media SET title = ?, subtitle = ?, publisher = ?, authors = ?, language = ?, categories = ?, isbns = ?, description = ?, medium_group = ?, medium_index = ?, main_category = ?, image = ? WHERE id = $id";
                    } else {
                        $query = "UPDATE media SET title = ?, subtitle = ?, publisher = ?, authors = ?, language = ?, categories = ?, isbns = ?, description = ?, medium_group = ?, medium_index = ?, main_category = ? WHERE id = $id";
                    }
                    ...
                    if(!mysqli_stmt_prepare($stmt, $query)){
                        ...
                        if(!$image_file_exists && $image != ""){
                            mysqli_stmt_bind_param($stmt, "ssssssssssss", $title, $subtitle, $publisher, $authors_string, $language, $categories_string, $isbns_string, $description, $medium_group, $medium_index, $main_category, $image);
                        } else {
                            mysqli_stmt_bind_param($stmt, "sssssssssss", $title, $subtitle, $publisher, $authors_string, $language, $categories_string, $isbns_string, $description, $medium_group, $medium_index, $main_category);
                        }
        ...
        if($_POST['submit'] == "Confirm return"){

            $query = "SELECT * FROM users WHERE id = (SELECT user_id FROM media WHERE id = $id)";
            $sql_result = mysqli_query($conn, $query);
            $returning_user = mysqli_fetch_assoc($sql_result);


            $query = "UPDATE media SET user_id = '-1', lent_until = NULL WHERE id = $id";
            if(!mysqli_query($conn, $query)){
                echo 'Database failed! [1]' . mysqli_error($conn);
            }
            $available = TRUE;
            $user_id = -1;
            ...
            $sql_result = mysqli_query($conn, $query);
            $returned_medium = mysqli_fetch_assoc($sql_result);
            new_log("Medium return confirmed: " . $title, array("medium" => $returned_medium, "user" => $returning_user));
        }

        //position stuff
        $sql_result = mysqli_query($conn, getSortingQuery($id));
        $sql_fetch = mysqli_fetch_assoc($sql_result);
        ...
        //sql request
        $sql_fetch = mysqli_fetch_all(mysqli_query($conn, "SELECT * FROM media WHERE id = $id"), MYSQLI_ASSOC);
        if(count($sql_fetch) > 1){
            echo 'Multiple[' . count($sql_fetch) . '] media with this id: [' . $id . '] !';
        } elseif(count($sql_fetch) == 0){
            echo 'No medium with this id: [' . $id . '] !';
        } else {
            $medium = $sql_fetch[0];
            ...
            //categories
            $categories = json_decode($medium['categories'])->categories;

            //authors
            $authors = json_decode($medium['authors'], JSON_OBJECT_AS_ARRAY)['authors'];

            //isbns
            $isbns = json_decode($medium['isbns'])->isbns;

            //isbn13 and isbn10
            foreach($isbns as $isbn){
                if($isbn->type == "ISBN_13"){
                    $isbn13 = $isbn->identifier;
                    $_SESSION['original_isbn13'] = $isbn13;
                } elseif($isbn->type == "ISBN_10"){
                    $isbn10 = $isbn->identifier;
                    $_SESSION['original_isbn10'] = $isbn10;
                }
            }
            ...
            //medium_group
            $medium_group = $medium['medium_group'];

            //volume
            if(isset($medium['volume'])){
            ...
            //medium_index
            $medium_index = $medium['medium_index'];

            //main_category
            $main_category = $medium['main_category'];
            ...
            //user id
            $_SESSION['user_id'] = $user_id = (int)$medium['user_id'];
            ...
            //status
            if($user_id != -1){
                $_SESSION['available'] = $available = FALSE;
            } else {
                $_SESSION['available'] = $available = TRUE;
            }

            //lent_until
            $_SESSION['lent_until'] = $lent_until = date_create($medium['lent_until']);

            //status_type
            if($available){
                $status_type = 'success';
            } elseif(date_diff($today, $lent_until)->days > 10){
                $status_type = 'info';
            } elseif(date_diff($today, $lent_until)->days > 5) {
                $status_type = 'warning';
            } else {
                $status_type = 'danger';
            }
    ...
    if(isset($_POST['reset_image'])){
        if(file_exists("images/media/" . $id . ".png")){
            unlink("images/media/" . $id . ".png");
        }
    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-lg-4 mb-3">
                <?php if(isset($id) && @getimagesize("images/media/" . $id . ".png")): ?>
                    <img src="<?php echo "images/media/" . $id . ".png" ?>" style="width:100%; max-width:300px; max-height: 300px;" alt="Sorry. No image found.">
                ...
                <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <input class="btn btn-outline-danger btn-sm mt-3" type="submit" name="reset_image" value="Reset Image">
                    <?php if(isset($image_msg)): ?>
                        <p class="text-danger"><small><?php echo $image_msg; ?></small></p>
                    <?php endif; ?>
                </form>
                <div class="card border border-<?php echo $status_type; ?> alert-<?php echo $status_type; ?>">
                    <h4 class="card-title text-center mt-3 mb-0">Status</h4>
                    <div class="card-body">
                        <table class="table table-sm text-center">
                            <tr>
                                <th>Status: </th>
                                <td class="font-weight-bold text-<?php echo $status_type; ?>"><?php echo $available ? 'Available' : 'Lent'; ?></td>
                            </tr>
                            <?php if(!$available): ?>
                                <tr>
                                    <th>User ID: </th>
                                ...
                                <tr>
                                    <th>User Name: </th>
                                    <td><?php echo $user['firstname'] . "<strong> " . $user['lastname'] . "</strong>"; ?></td>
                                </tr>
                                <tr>
                                    <th>User Email: </th>
                                ...
                                <tr>
                                    <th>Lent until: </th>
                                    <td><?php echo date_format($lent_until, "d.m.Y") . "<br>[" . date_diff($today, $lent_until)->days . " day(s)]"; ?></td>
                                </tr>
                            <?php endif; ?>
                        </table>
                    </div>
            ...
            <div class="col-lg-8">
                <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
                    <div class="form-group row">
                        ...
                        <input class="col-md-10 form-control bg-success border border-dark" value="<?php echo $id; ?>" type="text" id="id" readonly>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="card col-md-10">
                            <div class="row mt-3 mb-3 no-gutters">
                                <?php foreach($position_media as $position_medium): ?>
                                    <div class="col-sm">
                                        <div class="card border mx-1 rounded border-primary p-3 mb-3 mb-sm-0 <?php if($position_medium == "first" || $position_medium == "last"){ echo 'border-info'; } ?>" style="height: 300;">
                    ...
                    <div class="form-group row">
                        ...
                        <input class="col-md-10 form-control" value="<?php echo isset($publisher) ? $publisher : (isset($medium->volumeInfo->publisher) ? $medium->volumeInfo->publisher : ''); ?>" placeholder="Publisher" type="text" id="publisher" name="publisher">
                    </div>
                    <?php for($i = 0; $i < count($authors); $i++): ?>
                        ...
                        <div class="form-group row">
                            ...
                            <input class="col-md-10 form-control <?php if(($empty_error && $authors[$i]['author_lastname'] == "" && $authors[$i]['author_firstname'] == "") || ($empty_author_error && $authors[$i]['author_firstname'] == "" && $authors[$i]['author_lastname'] == "")){ echo 'alert-danger border border-danger';} elseif($author_warning){ echo 'alert-warning border border-warning';} elseif($i == 0){ echo 'alert-info border border-info';} ?>" value="<?php echo $authors[$i]['author_lastname']; ?>" placeholder="Lastname(s)" type="text" id="lastname" name="lastnames[<?php echo $i; ?>]">
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <input class="col-md-10 form-control" value="<?php echo $categories_string; ?>" placeholder="Categories" type="text" id="categories" name="categories">
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-10">
                            <div class="row justify-content-between">
                                <div class="col-md pl-0">
                                    <input class="form-control <?php if($medium_group_error){ echo 'alert-danger border border-danger'; } elseif($medium_group_warning){ echo 'alert-warning border border-warning';} ?>" value="<?php echo $medium_group; ?>" placeholder="Group" type="text" id="medium_group" name="medium_group">
                                </div>
                                <div class="col-md pr-0">
                                    <input class="form-control <?php if($volume_error){ echo 'alert-danger border border-danger'; } ?>" value="<?php echo $volume; ?>" placeholder="Volume" type="text" id="volume" name="volume">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row float-right">
                        <div class="col-auto">
                            <?php if(!$available): ?>
                                <input type="submit" value="Confirm return" name="submit" class="btn btn-info mb-3">
                            <?php endif; ?>
                        </div>
                        <div class="col-auto">
                            <?php if(!$already_exists): ?>
                                <input type="submit" value="Edit Medium" name="submit" class="btn btn-success d-inline float-right">
                            <?php else: ?>
                                <a href="medium.php?id=86" class="float-right"><button class="btn btn-outline-danger d-inline">Already exists in the database!</button></a>
                            <?php endif; ?>
